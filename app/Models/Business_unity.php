...
* @Since Version 1.0.0
*/
class Business_unity extends Model
{
    public $api;
    public $page;
    public $build;
    ...
    public function get_list()
    {
        ...
        if ($rol_id != 1 && !in_array('g_business', $this->session->get('permissions')))
        {
        ...
        if ($response->code == 200)
        {
            ...
            foreach ($rows as $row)
            {
                ...
                if ($rol_id == 1 || in_array('u_business', $this->session->get('permissions')))
                {
    ...
    {
        $params   = new stdClass();
        $endpoint = GET_BUSINESS_ROUTE . '/' . $id;
