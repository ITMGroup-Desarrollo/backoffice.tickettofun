...
const config = {
  loadData: function (response, band = false) {
    ...
    const scheduleSt = document.querySelectorAll('.hrStart')
    if (scheduleSt) {
  ...
  save: function (data) {
    ...
    const i = (parseInt(dataFilter.channel) === 3) ? 2 : 0

    const nodes = data.parentNode.parentNode.childNodes[i].childNodes[0]
  ...
  changeChannel: function (band = false) {
    ...
    switch (parseInt(channel.value)) {
      ...
      case 3:
        ...
        var id = (band === true && dataFilter.vendor_cruise !== '') ? dataFilter.vendor_cruise : null
    ...
    id = (band === true) ? dataFilter.vendor : null
  ...
  changeVendorCruise: function () {
    const channel = dataFilter.channel
    var cruise = document.querySelector('[name="cruise"]')
    const option = cruise.getElementsByTagName('option')
    option[0].value = ""

    if ((parseInt(channel) === 1 || parseInt(channel) === 3) && dataFilter.vendor_cruise !== '') {
      const id = (dataFilter.cruise !== '') ? dataFilter.cruise : null
      const vendor = dataFilter.vendor_cruise
