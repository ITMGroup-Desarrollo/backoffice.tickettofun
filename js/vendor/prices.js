...
if (reseller != null) {
  reseller.addEventListener('change', function (e) {
    ...
    var serviceEquivalence = document.querySelector('[name="service"]')
    ...
    if (search(this.value)) {
      for (var _iequivalence = 0; _iequivalence < equivalenceData.length; _iequivalence++) {
        if (this.value === equivalenceData[_iequivalence].reseller_id) {
          serviceEquivalence.innerHTML += '<option value="' + equivalenceData[_iequivalence].service_id + '">' + equivalenceData[_iequivalence].service_reseller + ' / <span style="color:red;">' + equivalenceData[_iequivalence].service_name + '<span></option>'
        }
      }
      serviceEquivalence.disabled = false
      document.querySelector('.save').disabled = false
      document.querySelector('[name="currency"]').disabled = false
      document.querySelector('[name="pax"]').disabled = false
      document.querySelector('[name="price"]').disabled = false
    } else {
      serviceEquivalence.innerHTML = '<option value="0">This Reseller has no assigned any equivalence</option>'
      serviceEquivalence.disabled = true
...
if (channel != null) {
  reseller = document.querySelector('[name="reseller"]')
...
form = document.querySelector('#add-price')
if (form != null) {
  var statusCombo = form.querySelector('[name="active"]')
